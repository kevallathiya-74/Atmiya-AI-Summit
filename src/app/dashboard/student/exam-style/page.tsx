...
import { PenTool, Filter, Clock, Sparkles, ChevronLeft, ChevronRight } from "lucide-react";
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-blue-50 p-4 md:p-8">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-6xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-bold text-gray-900">પરીક્ષા શૈલી પ્રશ્નો</h1>
            <p className="text-sm md:text-base text-gray-600 mt-2">Exam-Style Questions - Practice like real exams</p>
          </div>
                ...
                <Card className="p-6 hover:shadow-lg transition-all">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex gap-2">
                      <span className="px-3 py-1 bg-blue-100 text-blue-700 text-xs font-bold rounded">Q{q.id}</span>
                      <span className={cn("px-3 py-1 text-xs font-bold rounded", q.difficulty === "Easy" ? "bg-green-100 text-green-700" : q.difficulty === "Medium" ? "bg-yellow-100 text-yellow-700" : "bg-red-100 text-red-700")}>{q.difficulty}</span>
                    </div>
                    <div className="flex gap-4 text-sm text-gray-600">
                      <span>{q.marks} marks</span>
                      <span className="flex items-center gap-1"><Clock className="w-4 h-4" />{q.time}min</span>
                    </div>
                  </div>
